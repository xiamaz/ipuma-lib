...
class IPUMultiDriver {
 ...
 public:
  ...
  void fill_input_buffer(const ipu::RawSequences &a, const ipu::RawSequences &b,
                         std::vector<int32_t> &input_buffer,
                         std::vector<int32_t> &mapping) {
    ipu::batchaffine::SWAlgorithm::prepare_remote(
        config.swconfig, config.ipuconfig, a, b, &*input_buffer.begin(),
        &*input_buffer.end(), mapping.data());
  }

  ipu::batchaffine::Job *submit(std::vector<int32_t> &inputBuffer, std::vector<int32_t> &resultBuffer) {
    auto *job = driver->async_submit_prepared_remote_compare(
        &*inputBuffer.begin(), &*inputBuffer.end(), &*resultBuffer.begin(),
        &*resultBuffer.end());
    return job;
  }
  ...
  void wait(ipu::batchaffine::Job *job) {
    ...
    auto timeJob =
        static_cast<double>(job->tick.accumulate_microseconds()) / 1e6;
    auto timeBatch =
        static_cast<double>(job->sb.runTick.accumulate_microseconds()) / 1e6;
    ...
    for (size_t i = 0; i < config.ipuconfig.getTotalNumberOfComparisons();
         i++) {
  ...
  void submitWait(std::vector<int32_t> &inputBuffer, std::vector<int32_t> &resultBuffer) {
    auto *job = submit(inputBuffer, resultBuffer);
    wait(job);
    delete job;
  }
  ...
  void fillResults(std::vector<int32_t> &resultBuffer, std::vector<int> &mappingBuffer, ipu::batchaffine::BlockAlignmentResults &results, const int numCmps) {
    ...
    ipu::batchaffine::SWAlgorithm::transferResults(
        &*resultBuffer.begin(), &*resultBuffer.end(), &*mappingBuffer.begin(),
        &*mappingBuffer.end(), &*(results.scores.begin()),
        &*(results.scores.begin() + numCmps),
        &*(results.a_range_result.begin()),
        &*(results.a_range_result.begin() + numCmps),
        &*(results.b_range_result.begin()),
        &*(results.b_range_result.begin() + numCmps), numCmps);
  }
};
